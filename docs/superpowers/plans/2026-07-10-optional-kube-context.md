# Optional Kubernetes Context Historical Implementation Plan

> **Historical record:** This records the TDD sequence used to implement the
> merged behavior. Code snippets describe the resulting interfaces and tests;
> they are not instructions to replay commits.

**Goal:** Add optional `--kube-context` support to `run-suite` alongside
`--cluster-name`, so existing contexts can be targeted explicitly while
preserving the credential-refresh workflow by default.
...
- Preserve `--cluster-name` as an optional override of the suite-derived AKS name.
...
- Every mode loads typed requirements, validates the `aks` provider, and validates `infrastructure.nodePools` against node-selector requirements before side effects.
- Image-build runs read generated deployment outputs and require the requested cluster to match the managed deployment so its kubelet identity owns `AcrPull` on the registry.
- Record explicit context as `kubeContext`; omit `kubeContext` and preserve current `clusterName` metadata in legacy mode.
- Omit `clusterName` in explicit metadata because `kubeContext` identifies the Kubernetes target; the resolved cluster name is used only for image-build deployment validation.
- Do not change `provision`, `destroy`, Azure ownership, image override, experiment, or latency-reporting behavior because of optional context. Provision continues to generate ARM parameters from typed requirements.
...
- Preserves optional `run-suite --cluster-name NAME` for overriding the name derived from the suite.
- Produces `validateRunSuiteTarget(kubeContext, resourceGroup string, builds []acr.ImageBuild) error`.
- Consumes typed `requirements.Load` results, including `infrastructure.nodePools`.
- Uses generated deployment outputs for image-build cluster ownership and registry discovery.
...
Add a helper in `cmd/perf-runner/main_test.go` that creates a minimum valid
suite with typed infrastructure requirements. The fixture includes the system
and user pools referenced by node-selector requirements:
...
Make fake kubectl return a valid server version and node result, and make fake
kube-burner exit successfully. Call:
...
and returning `<child>/metadata/run.yml`. The empty Azure marker proves that
explicit no-build mode performs no deployment-output query, image build, or
credential refresh while still loading and validating typed requirements.

- [ ] **Step 3: Add failing legacy credential and invalid-combination tests**

Use the same typed requirements fixture for a legacy suite, run with
`--resource-group rg-test`, and assert `azMarker` contains
`aks get-credentials --resource-group rg-test --name aksexisting
--overwrite-existing`. Add a `--cluster-name existing-aks` variant and assert
that credentials use the override. Assert the kubectl marker contains
`version -o json` but not `--context`.
...
rg-build`. Use fake `az` and `kubectl` commands plus a suite-local build context.
Return `clusterName`, `containerRegistryName`, and
`containerRegistryLoginServer` from the fake managed deployment. Assert the
resolved or overridden cluster name matches the deployment before registry
outputs and ACR build operations proceed, but that the Azure marker does not
contain `aks get-credentials`; assert every Kubernetes marker line contains
`--context preview`.

Add focused tests for the image-build preparation helper. Verify deployment
outputs are read in this order:

```text
clusterName, containerRegistryName, containerRegistryLoginServer
```

Verify a managed-deployment cluster mismatch fails after `clusterName` and
before registry output reads, credential refresh, or image builds. The error
must explain that the deployment owns the kubelet identity's `AcrPull` access.
...
- [ ] **Step 6: Implement CLI and typed requirement validation before side effects**
...
Load the typed requirements in every mode and validate the provider. Derive the
image-build list, validate the mode-specific resource-group rule, then derive
the cluster name and validate node-pool/selector relationships. All of these
checks complete before external commands or run-directory creation:
...
- [ ] **Step 7: Derive cluster identity and prepare mode-specific Azure state**
...
registryName, registryServer, err := prepareRunSuiteCluster(
	ctx,
	*resourceGroup,
	clusterName,
	req.Requires.Images,
...
`prepareRunSuiteCluster` has three paths:

- legacy no-build refreshes credentials for the derived or overridden cluster;
- explicit no-build performs no Azure calls;
- any image-build mode first reads the generated managed-deployment
  `clusterName` output, rejects ownership mismatches, then reads registry
  outputs; only legacy mode subsequently refreshes credentials.

After Azure preparation, use `target.Output` for Kubernetes version and selector
validation. This keeps all modes on typed requirements while making every
explicit mode independent of credential refresh.
...
runpkg.Metadata{
	...
	ResourceGroup: *resourceGroup, ClusterName: metadataClusterName, KubeContext: *kubeContext,
...
- Consumes: optional `run-suite --cluster-name` from the merged CLI.
- Produces: optional `KUBE_CONTEXT` Make variable with run-suite-only resource-group suppression.
- Produces: optional `CLUSTER_NAME` Make variable shared by `provision` and `run-suite`.
...
Add a provision assertion that `CLUSTER_NAME=custom-aks` produces
`--cluster-name "custom-aks"`, while the default invocation omits the flag.
...
Append `$(CLUSTER_NAME_ARG)` to `provision`; do not change `destroy`.
`$(origin RESOURCE_GROUP)` equals `file` only for the Makefile's `?=` default;
command-line and environment values remain caller-supplied and are forwarded.
...
Document that explicit context skips `az aks get-credentials`, targets every
kubectl and kube-burner operation, and may omit `RESOURCE_GROUP` only for suites
without image builds. State that suites with image builds still require a
resource group, a derived or overridden cluster name matching the managed
deployment, and registry outputs for ACR access. Document `CLUSTER_NAME` for
both provision and run-suite. State that no separate kubeconfig option is
supported and that optional context does not alter provision or destroy
semantics.
