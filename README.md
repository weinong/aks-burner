...
Existing AKS patchpools whose desired configuration still contains `perf.azure.com/kata-shim-patch=pending:NoSchedule` must be migrated through the AKS control plane before this manifest can roll out. Clear the desired taints with `az aks nodepool update --resource-group <resource-group> --cluster-name <cluster-name> --name patchpool --node-taints ""`, or reprovision/recreate the pool. Removing the taint only through `kubectl` is not durable because AKS can restore the control-plane desired configuration. Newly provisioned `kata-io` pools already declare empty taints.
